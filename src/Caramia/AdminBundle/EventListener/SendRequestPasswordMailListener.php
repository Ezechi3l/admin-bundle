...
namespace Caramia\AdminBundle\EventListener;
...
class SendRequestPasswordMailListener
{
    ...
    public function onRequestedPassword(UserDataEvent $event)
    {
        ...
        $message = \Swift_Message::newInstance()
            ...
            ->setFrom($this->from)
            ->setTo($user->getEmail())
            ->setBody($this->templating->loadTemplate($this->template)->renderBlock(
                'body', [
                    'username' => $user->getUsername(),
                    'request_link' => $this->router->generate('reset_password', ['token' => $token], true)
                ]
            ), 'text/html');
